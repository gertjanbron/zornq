...
\author{%
  Gertjan Bron\thanks{Corresponding author: \href{mailto:gertjanbron@gmail.com}{gertjanbron@gmail.com}}\\
...
\begin{table}[t]
  ...
  \begin{tabular}{llll}
    ...
    FW-\SDP sandwich    & Anytime $\LB \le \OPT \le \UB$ & Near-exact / Bounded & $n \lesssim 500$ \\
    ...
    \QAOA (Aer / runtime) & Quantum LB, hardware-matched & Heuristic   & $n \lesssim 30$ (sim), $n \le 127$ (hw) \\
    \bottomrule
  \end{tabular}
...
graphs $n = 30\,\text{--}\,500$ the \FW solver matches \textsc{cvxpy}
on all cases where the latter remains feasible ($n \le 300$), and is
$3.9\times$ faster at $n = 200$; for $n > 300$, \textsc{cvxpy} runs out
of memory and \FW continues to deliver sandwich certificates.
...
enumeration on $K_{18}$ ($11.6$\,ms vs.\ $6226$\,ms) and matches the
...
\begin{equation}
  ...
  \qquad \text{gap} = 7.61\%.
\end{equation}
This is the first publicly-documented sandwich on \texttt{myciel3} at
...
Results are persisted to \texttt{docs/paper/hardware/jobs/<job\_id>.json}
and are replayable via \texttt{--resume <job\_id>}. We follow the spirit
